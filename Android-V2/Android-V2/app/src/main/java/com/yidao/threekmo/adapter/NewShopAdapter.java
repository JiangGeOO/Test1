...
public class NewShopAdapter extends BaseRecyclerAdapter<ShopDetailResult> {
    ...
    public NewShopAdapter(Context mContext) {
        super(mContext);
    ...
    @Override
    public void onBind(RecyclerView.ViewHolder viewHolder, int RealPosition, ShopDetailResult shopDetailResult) {
        MyHolder myHolder = (MyHolder) viewHolder;
        if(shopDetailResult != null){
            Glide.with(mContext).load(shopDetailResult.getPhotoUrl()).into(myHolder.item_image);

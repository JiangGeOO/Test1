...
abstract class BaseListActivity<Bind : ViewDataBinding, Bean, Model : IResponseData<Bean>> : IBaseActivity<Bind>() {
    ...
    override fun initViewAndData() {
        ...
        iDataInterface = object : IDataInterface<Bean, Model>(activity, bindingInfo) {
            ...
            override fun dataLoadFailed(throwable: Throwable) {
                ...
                this@BaseListActivity.dataLoadFailed(throwable)
            }

            override fun onDataGet(data: Model?): Boolean {
                return if (activity == null) false else this@BaseListActivity.onDataGet(data)
            }

            override fun onDataShow(data: ArrayList<Bean>?) {
                if (activity == null) return
                this@BaseListActivity.onDataShow(data)
            }

            override fun afterBindToView(model: Model?) {
                this@BaseListActivity.afterBindToView(model)
            }

            override fun addHeader() {
                this@BaseListActivity.addHeader()
            }

            override fun addFooter() {
                this@BaseListActivity.addFooter()
            }
        }
